...
def load_elliptic_dataset(data_dir, lang):
    ...
    result = pkl.load(open('{}/{}/{}.pkl'.format(data_dir, 'processed', lang), 'rb'))
    ...
    """
    # use torch geometric data instead
    data = Data(x=node_feat, edge_index=edge_index, y=torch.tensor(label), mask=torch.tensor(label)>=0)
    return data
