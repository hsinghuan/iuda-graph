...
class MultigraphFixMatchAdapter(FullModelMultigraphAdapter):
    ...
    def _adapt_train_epoch(self, model, tgt_train_loader, optimizer, thres, con_tradeoff, scheduler=None):
        ...
        for _ in range(len_dataloader):
            if self.src_train_loader:
                ...
                # src_con_loss = self._fixmatch_loss(model, src_data, thres, return_logits=False)
                total_src_logits.append(src_y)
            else:
                src_loss = torch.tensor(0.0)
                # src_con_loss = torch.tensor(0.0)
            ...
            tgt_con_loss, tgt_logits = self._fixmatch_loss(model, tgt_data, thres)
            total_tgt_logits.append(tgt_logits)
            loss = src_loss + tgt_con_loss * con_tradeoff
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            if scheduler:
            ...
            total_con_loss += tgt_con_loss.item()
    ...
    def _adapt_test_epoch(self, model, tgt_val_loader, thres, con_tradeoff):
        ...
        for _ in range(len_dataloader):
            ...
            # if 'node_mask' in tgt_data:
            #     pseudo_tgt_mask = torch.logical_and(pseudo_tgt_mask, tgt_data.node_mask)
            # tgt_con_loss = F.nll_loss(F.log_softmax(tgt_y[pseudo_tgt_mask], dim=1),
            #                       pseudo_tgt_label[
            #                           pseudo_tgt_mask]) if pseudo_tgt_mask.sum().item() != 0 else torch.tensor(0.)
            tgt_con_loss, tgt_logits = self._fixmatch_loss(model, tgt_data, thres)
            total_tgt_logits.append(tgt_logits)
    ...
    def _fixmatch_loss(self, model, data, thres, return_logits=True):
        pseudo_label, pseudo_mask, pseudo_logits = self._pseudo_label(model, self.weak_augment(deepcopy(data)), thres)
        # total_tgt_logits.append(pseudo_tgt_logits)

        # strong augment
        data_s = self.strong_augment(deepcopy(data))
        y_s, _ = model(data_s.x, data_s.edge_index)
        if 'node_mask' in data_s:
            pseudo_mask = torch.logical_and(pseudo_mask, data_s.node_mask)
        loss = F.nll_loss(F.log_softmax(y_s[pseudo_mask], dim=1), pseudo_label[pseudo_mask]) if pseudo_mask.sum().item() != 0 else torch.tensor(0.)

        if return_logits:
            return loss, pseudo_logits
        else:
            return loss
...
class SinglegraphFixMatchAdapter(FullModelSinglegraphAdapter):
    ...
    def _adapt_train_epoch(self, model, tgt_data, optimizer, thres, con_tradeoff, scheduler=None):
        ...
        if self.src_data:
            ...
            # src_con_loss = self._fixmatch_loss(model, self.src_data, thres, train=True, return_logits=False)
        ...
        tgt_con_loss, tgt_logits = self._fixmatch_loss(model, tgt_data, thres, train=True)
        ...
        return loss.item(), src_loss.item(), tgt_con_loss.item(), src_logits, tgt_logits[tgt_data.train_mask]
    ...
    def _adapt_test_epoch(self, model, tgt_data, thres, con_tradeoff):
        ...
        if self.src_data:
            ...
            # src_con_loss = self._fixmatch_loss(model, self.src_data, thres, train=False, return_logits=False)
        ...
        #     0.)
        tgt_con_loss, tgt_logits = self._fixmatch_loss(model, tgt_data, thres, train=False)

        loss = src_loss + tgt_con_loss * con_tradeoff

        return loss.item(), src_loss.item(), tgt_con_loss.item(), src_logits, tgt_logits[tgt_data.val_mask]
    ...
    def adapt(self, tgt_data, threshold_list, con_tradeoff_list, stage_name, args, subdir_name="", k_hop=2):

        # subsample k-hop subgraph of target nodes so that all augmentations would only be applied at this subgraph
        # print(f"Subsample {k_hop}-hop subgraph of target nodes")
        # tgt_x, tgt_edge_index = tgt_data.x, tgt_data.edge_index
        # tgt_train_mask, tgt_val_mask = tgt_data.train_mask, tgt_data.val_mask
        # tgt_train_node_idx = torch.argwhere(tgt_train_mask).squeeze(1).tolist()
        # tgt_val_node_idx = torch.argwhere(tgt_val_mask).squeeze(1).tolist()
        # subset, new_tgt_edge_index, mapping, edge_mask = k_hop_subgraph(tgt_train_node_idx + tgt_val_node_idx, k_hop, tgt_edge_index, relabel_nodes=True)
        # new_tgt_x = tgt_x[subset]
        # new_tgt_train_mask = torch.zeros_like(subset, dtype=torch.bool)
        # new_tgt_train_mask[mapping[:len(tgt_train_node_idx)]] = True # first half of the mapping are training nodes
        # new_tgt_val_mask = torch.zeros_like(subset, dtype=torch.bool)
        # new_tgt_val_mask[mapping[len(tgt_train_node_idx):]] = True # second half of the mapping are validation nodes
        # tgt_data = Data(x=new_tgt_x, edge_index=new_tgt_edge_index, train_mask=new_tgt_train_mask, val_mask=new_tgt_val_mask)
    ...
    def _fixmatch_loss(self, model, data, thres, train=True, return_logits=True):
        # weak augment
        pseudo_label, pseudo_mask, pseudo_logits = self._pseudo_label(model, self.weak_augment(deepcopy(data)), thres)
        # strong augment
        data_s = self.strong_augment(deepcopy(data))
        y_s, _ = model(data_s.x, data_s.edge_index)
        if train:
            pseudo_mask = torch.logical_and(pseudo_mask, data.train_mask)
        else:
            pseudo_mask = torch.logical_and(pseudo_mask, data.val_mask)
        if 'node_mask' in data_s:
            pseudo_mask = torch.logical_and(pseudo_mask, data_s.node_mask)

        loss = F.nll_loss(F.log_softmax(y_s[pseudo_mask], dim=1),
                              pseudo_label[pseudo_mask]) if pseudo_mask.sum().item() != 0 else torch.tensor(0.)
