...
class MultigraphGCSTFPLDirect(DecoupledMultigraphAdapter):
    ...
    def _adapt_train_test(self, tgt_train_loader, tgt_val_loader, thres, contrast, args):
        ...
        for e in range(1, args.adapt_epochs + 1):
            train_loss, train_src_loss, train_tgt_loss, train_src_contrast_loss, train_tgt_contrast_loss, train_src_logits, train_tgt_logits = self._adapt_train_epoch(
                encoder, classifier, dgi, tgt_pseudo_train_loader, optimizer, contrast)
            val_loss, val_src_loss, val_tgt_loss, val_src_contrast_loss, val_tgt_contrast_loss, val_src_logits, val_tgt_logits = self._adapt_test_epoch(
                ...
                tgt_pseudo_val_loader, contrast)
            ...
            #         break
            #
            # for name, param in dgi.encoder1.named_parameters():
            #     if param.requires_grad:
            #         print(name, param.data[0])
            #         break
    ...
    def adapt(self, tgt_train_loader, tgt_val_loader, threshold_list, contrast_list, stage_name, args, subdir_name=""):
        ...
        self._set_adapted_encoder_classifier(best_encoder, best_classifier)

    def _set_adapted_encoder_classifier(self, encoder, classifier):
        self.adapted_encoder = deepcopy(encoder).to(self.device)
        self.adapted_classifier = deepcopy(classifier).to(self.device)

    def get_adapted_encoder_classifier(self):
        return self.adapted_encoder, self.adapted_classifier




class SinglegraphGCSTFPLDirect(DecoupledSinglegraphAdapter):
    ...
    def _adapt_train_epoch(self, encoder, classifier, dgi, src_loader, tgt_loader, optimizer, contrast):
        ...
        for _ in range(len_dataloader):
            ...
            pseudo_tgt_label = torch.squeeze(pseudo_tgt_label, dim=0).to(self.device)
            pseudo_tgt_mask = torch.squeeze(pseudo_tgt_mask, dim=0).to(self.device)
            pseudo_tgt_mask = torch.logical_and(pseudo_tgt_mask, tgt_data.train_mask)
            tgt_data = tgt_data.to(self.device)
            tgt_y, _ = classifier(encoder(tgt_data.x, tgt_data.edge_index))
            tgt_loss = F.nll_loss(F.log_softmax(tgt_y, dim=1)[pseudo_tgt_mask],
                                  pseudo_tgt_label[pseudo_tgt_mask], reduction='sum')
    ...
    def _adapt_test_epoch(self, encoder, classifier, dgi, src_loader, tgt_loader, contrast):
        ...
        for _ in range(len_dataloader):
            ...
            pseudo_tgt_label = torch.squeeze(pseudo_tgt_label, dim=0).to(self.device)
            pseudo_tgt_mask = torch.squeeze(pseudo_tgt_mask, dim=0).to(self.device)
            pseudo_tgt_mask = torch.logical_and(pseudo_tgt_mask, tgt_data.val_mask)
            tgt_data = tgt_data.to(self.device)
            tgt_y, _ = classifier(encoder(tgt_data.x, tgt_data.edge_index))
            tgt_loss = F.nll_loss(F.log_softmax(tgt_y, dim=1)[pseudo_tgt_mask],
                                  pseudo_tgt_label[pseudo_tgt_mask], reduction='sum')
    ...
    def _adapt_train_test(self, src_loader, tgt_loader, thres, contrast, args):
        encoder = deepcopy(self.encoder)
        classifier = deepcopy(self.classifier)
        # pseudo_y_hard_label, pseudo_mask = self._pseudo_label(encoder, classifier, tgt_data, thres)
        tgt_datalist = []
        for data in tgt_loader:
            data = data.to(self.device)
            pseudo_y_hard_label, pseudo_mask = self._pseudo_label(encoder, classifier, data, thres)
            tgt_datalist.append((data, pseudo_y_hard_label, pseudo_mask))
        tgt_pseudo_loader = DataLoader(dataset=tgt_datalist, batch_size=1, shuffle=True)
        ...
        for e in range(1, args.adapt_epochs + 1):
            train_loss, train_src_loss, train_tgt_loss, train_src_contrast_loss, train_tgt_contrast_loss, train_src_logits, train_tgt_logits = self._adapt_train_epoch(
                encoder, classifier, dgi, src_loader, tgt_pseudo_loader, optimizer, contrast)
            val_loss, val_src_loss, val_tgt_loss, val_src_contrast_loss, val_tgt_contrast_loss, val_src_logits, val_tgt_logits = self._adapt_test_epoch(
                encoder, classifier, dgi,
                src_loader, tgt_pseudo_loader, contrast)
            train_src_score = self.validator(target_train={'logits': train_src_logits})
            train_tgt_score = self.validator(target_train={'logits': train_tgt_logits})
            val_src_score = self.validator(target_train={'logits': val_src_logits})
            val_tgt_score = self.validator(target_train={'logits': val_tgt_logits})
