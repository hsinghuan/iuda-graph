...
class MultigraphGCSTFPLXCON(DecoupledMultigraphAdapter):
    ...
    def adapt(self, tgt_train_loader, tgt_val_loader, threshold_list, stage_name, args, subdir_name=""):
        ...
        for thres in threshold_list:
            ...
            performance_dict[thres] = {'encoder': encoder, 'classifier': classifier, 'val_score': val_score}

        best_val_score = -np.inf
        best_encoder, best_classifier = None, None
        for thres, ckpt_dict in performance_dict.items():
            ...
            print(f"thres: {thres} val_score: {ckpt_dict['val_score']}")
...
class SinglegraphGCSTFPLXCON(DecoupledSinglegraphAdapter):
    def __init__(self, encoder, classifier, emb_dim, src_data=None, device="cpu"):
        super().__init__(encoder, classifier, src_data, device)
        self.emb_dim = emb_dim
        self.validator = IMValidator()

    def _adapt_train_epoch(self, encoder, classifier, src_loader, tgt_loader, optimizer):
        encoder.train()
        classifier.train()
    ...
    @torch.no_grad()
    def _adapt_test_epoch(self, encoder, classifier, src_loader, tgt_loader):
        encoder.eval()
        classifier.eval()
    ...
    def _adapt_train_test(self, src_loader, tgt_loader, thres, args):
        encoder = deepcopy(self.encoder)
        classifier = deepcopy(self.classifier)
        ...
        for e in range(1, args.adapt_epochs + 1):
            train_loss, train_src_loss, train_tgt_loss, train_src_logits, train_tgt_logits = self._adapt_train_epoch(
                encoder, classifier, src_loader, tgt_pseudo_loader, optimizer)
            val_loss, val_src_loss, val_tgt_loss, val_src_logits, val_tgt_logits = self._adapt_test_epoch(
                encoder, classifier,
                src_loader, tgt_pseudo_loader)
            train_src_score = self.validator(target_train={'logits': train_src_logits})
            train_tgt_score = self.validator(target_train={'logits': train_tgt_logits})
            val_src_score = self.validator(target_train={'logits': val_src_logits})
            val_tgt_score = self.validator(target_train={'logits': val_tgt_logits})
    ...
    def adapt(self, tgt_data, threshold_list, stage_name, args, subdir_name=""):
        ...
        for thres in threshold_list:
            ...
            self.writer = SummaryWriter(
                os.path.join(args.log_dir, subdir_name, stage_name,
                             run_name))
            encoder, classifier, val_score = self._adapt_train_test(cluster_src_loader, cluster_tgt_loader, thres, args)
            performance_dict[thres] = {'encoder': encoder, 'classifier': classifier, 'val_score': val_score}
