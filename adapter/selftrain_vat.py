...
class SinglegraphVirtualAdversarialSelfTrainer(FullModelSinglegraphAdapter):
    ...
    def _adapt_train_epoch(self, model, tgt_data, pseudo_tgt_label, pseudo_tgt_mask, optimizer, xi=1e-6, eps=1.0, ip=1):
        model.train()

        vat_loss = VATLoss(xi=xi, eps=eps, ip=ip)
        if self.src_data:
            src_node_num = self.src_data.train_mask.sum().item()
            src_lds = vat_loss(model, self.src_data, mask=self.src_data.train_mask) * src_node_num
        ...
        tgt_lds = vat_loss(model, tgt_data, mask=tgt_data.train_mask) * tgt_node_num
        tgt_y, _ = model(tgt_data.x, tgt_data.edge_index)
        tgt_loss = F.nll_loss(F.log_softmax(tgt_y, dim=1)[pseudo_tgt_train_mask], pseudo_tgt_label[pseudo_tgt_train_mask], reduction='sum')
        ...
        train_lds = (src_lds.item() + tgt_lds.item()) / (src_node_num + tgt_node_num)  if (src_node_num + tgt_node_num) > 0 else 0.
        return train_loss, train_src_loss, train_tgt_loss, train_lds, src_logits, tgt_logits

    def _adapt_test_epoch(self, model, tgt_data, pseudo_tgt_label, pseudo_tgt_mask, xi=1e-6, eps=1.0, ip=1):
        model.eval()

        vat_loss = VATLoss(xi=xi, eps=eps, ip=ip)
        if self.src_data:
            src_node_num = self.src_data.val_mask.sum().item()
            src_lds = vat_loss(model, self.src_data, mask=self.src_data.val_mask) * src_node_num
            src_y, _ = model(self.src_data.x, self.src_data.edge_index)
        ...
        tgt_lds = vat_loss(model, tgt_data, mask=tgt_data.val_mask) * tgt_node_num
        tgt_y, _ = model(tgt_data.x, tgt_data.edge_index)
        ...
        val_lds = (src_lds.item() + tgt_lds.item()) / (src_node_num + tgt_node_num) if (src_node_num + tgt_node_num) > 0 else 0.
        return val_loss, val_src_loss, val_tgt_loss, val_lds, src_logits, tgt_logits 

    def _adapt_train_test(self, tgt_data, eps, args):
        thres = 0.0
        model = deepcopy(self.model)
        ...
        for e in range(1, args.adapt_epochs + 1):
            train_loss, train_src_loss, train_tgt_loss, train_lds, train_src_logits, train_tgt_logits = self._adapt_train_epoch(
                model, tgt_data, pseudo_y_hard_label, pseudo_mask, optimizer, eps=eps)
            val_loss, val_src_loss, val_tgt_loss, val_lds, val_src_logits, val_tgt_logits = self._adapt_test_epoch(
                model, tgt_data, pseudo_y_hard_label, pseudo_mask, eps=eps)
            train_src_score = self.validator(target_train={'logits': train_src_logits})
            train_tgt_score = self.validator(target_train={'logits': train_tgt_logits})
            val_src_score = self.validator(target_train={'logits': val_src_logits})
            val_tgt_score = self.validator(target_train={'logits': val_tgt_logits})
    ...
    def adapt(self, tgt_data, eps_list, stage_name, args, subdir_name=""):
        tgt_data = tgt_data.to(self.device)
        performance_dict = dict()
        for eps in eps_list:
            run_name = f'{args.method}_{str(eps)}_{str(args.model_seed)}'
            self.writer = SummaryWriter(os.path.join(args.log_dir, subdir_name, stage_name, run_name))
            model, val_score = self._adapt_train_test(tgt_data, eps, args)
            performance_dict[eps] = {'model': model, 'val_score': val_score}
...
class VATLoss(nn.Module):
    ...
    def forward(self, model, data, mask=None):
        with torch.no_grad():
