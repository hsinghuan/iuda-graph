...
class MultigraphGCSTXPL(DecoupledMultigraphAdapter):
    ...
    def _adapt_train_epoch(self, encoder, classifier, dgi, tgt_train_loader, optimizer, contrast):
        encoder.train()
        classifier.train()
        dgi.train()
        ...
        for _ in range(len_dataloader):
            ...
            loss = src_loss + contrast * (src_contrast_loss + tgt_contrast_loss)
        ...
        total_loss = (total_src_loss + contrast * (total_src_contrast_loss + total_tgt_contrast_loss)) / (total_src_node + total_src_contrast_num + total_tgt_contrast_num)  if (total_src_node + total_src_contrast_num + total_tgt_contrast_num) > 0 else 0.
        ...
        return total_loss, total_src_loss, total_src_contrast_loss, total_tgt_contrast_loss, total_src_logits, total_tgt_logits

    @torch.no_grad()
    def _adapt_test_epoch(self, encoder, classifier, dgi, tgt_val_loader, contrast):
        encoder.eval()
        classifier.eval()
        dgi.eval()
        ...
        total_loss = (total_src_loss + contrast * (
                    total_src_contrast_loss + total_tgt_contrast_loss)) / (
                                 total_src_node + total_src_contrast_num + total_tgt_contrast_num) if ( total_src_node + total_src_contrast_num + total_tgt_contrast_num) > 0 else 0.
        total_src_loss = total_src_loss / total_src_node if total_src_node > 0 else 0.
        total_src_contrast_loss = total_src_contrast_loss / total_src_contrast_num if total_src_contrast_num > 0 else 0.
        total_tgt_contrast_loss = total_tgt_contrast_loss / total_tgt_contrast_num if total_tgt_contrast_num > 0 else 0.
        total_src_logits = torch.cat(total_src_logits)
    ...
    def _adapt_train_test(self, tgt_train_loader, tgt_val_loader, contrast, args):
        ...
        for e in range(1, args.adapt_epochs + 1):
            train_loss, train_src_loss, train_src_contrast_loss, train_tgt_contrast_loss, train_src_logits, train_tgt_logits = self._adapt_train_epoch(
                encoder, classifier, dgi, tgt_train_loader, optimizer, contrast)
            val_loss, val_src_loss, val_src_contrast_loss, val_tgt_contrast_loss, val_src_logits, val_tgt_logits = self._adapt_test_epoch(
                encoder, classifier, dgi,
                tgt_val_loader, contrast)
            ...
            if val_loss < best_val_loss:
                ...
                staleness += 1
            print(
                f'Contrast: {contrast} Epoch: {e} Train Loss: {round(train_loss, 3)} Train Src Loss: {round(train_src_loss, 3)} Train Src Contrast Loss: {round(train_src_contrast_loss, 3)} Train Tgt Contrast Loss: {round(train_tgt_contrast_loss, 3)} \n Val Loss: {round(val_loss, 3)} Val Src Loss: {round(val_src_loss, 3)} Val Src Contrast Loss: {round(val_src_contrast_loss, 3)} Val Tgt Contrast Loss: {round(val_tgt_contrast_loss, 3)}')
    ...
    def adapt(self, tgt_train_loader, tgt_val_loader, contrast_list, stage_name, args, subdir_name=""):
        performance_dict = defaultdict(dict)
        for contrast in contrast_list:
            run_name = f'{args.method}_{str(contrast)}_{str(args.model_seed)}'
            self.writer = SummaryWriter(
                os.path.join(args.log_dir, subdir_name, stage_name,
                             run_name))
            encoder, classifier, val_score = self._adapt_train_test(tgt_train_loader, tgt_val_loader, contrast, args)
            performance_dict[contrast] = {'encoder': encoder, 'classifier': classifier, 'val_score': val_score}

        best_val_score = -np.inf
        best_encoder, best_classifier = None, None
        for contrast, ckpt_dict in performance_dict.items():
            ...
            print(f"contrast: {contrast} val_score: {ckpt_dict['val_score']}")

        self.set_encoder_classifier(best_encoder, best_classifier)



class SinglegraphGCSTXPL(DecoupledSinglegraphAdapter):
    def __init__(self, encoder, classifier, emb_dim, src_data=None, device="cpu"):
        super().__init__(encoder, classifier, src_data, device)
        self.emb_dim = emb_dim
        self.validator = IMValidator()

    def _adapt_train_epoch(self, encoder, classifier, dgi, src_loader, tgt_loader, optimizer, contrast):
        encoder.train()
        classifier.train()
        dgi.train()
    ...
    @torch.no_grad()
    def _adapt_test_epoch(self, encoder, classifier, dgi, src_loader, tgt_loader, contrast):
        encoder.eval()
        classifier.eval()
        dgi.eval()
        ...
        total_loss = (total_src_loss + contrast * (
                total_src_contrast_loss + total_tgt_contrast_loss)) / (
                             total_src_node + total_src_contrast_num + total_tgt_contrast_num) if (
                                                                                                                           total_src_node + total_src_contrast_num + total_tgt_contrast_num) > 0 else 0.
        total_src_loss = total_src_loss / total_src_node if total_src_node > 0 else 0.
        total_src_contrast_loss = total_src_contrast_loss / total_src_contrast_num if total_src_contrast_num > 0 else 0.
        total_tgt_contrast_loss = total_tgt_contrast_loss / total_tgt_contrast_num if total_tgt_contrast_num > 0 else 0.
        total_src_logits = torch.cat(total_src_logits)
    ...
    def _adapt_train_test(self, src_loader, tgt_loader, contrast, args):
        encoder = deepcopy(self.encoder)
        classifier = deepcopy(self.classifier)
        ...
        for e in range(1, args.adapt_epochs + 1):
            train_loss, train_src_loss, train_src_contrast_loss, train_tgt_contrast_loss, train_src_logits, train_tgt_logits = self._adapt_train_epoch(
                encoder, classifier, dgi, src_loader, tgt_loader, optimizer, contrast)
            val_loss, val_src_loss, val_src_contrast_loss, val_tgt_contrast_loss, val_src_logits, val_tgt_logits = self._adapt_test_epoch(
                encoder, classifier, dgi,
                src_loader, tgt_loader, contrast)
            train_src_score = self.validator(target_train={'logits': train_src_logits})
            train_tgt_score = self.validator(target_train={'logits': train_tgt_logits})
            val_src_score = self.validator(target_train={'logits': val_src_logits})
            val_tgt_score = self.validator(target_train={'logits': val_tgt_logits})
    ...
    def adapt(self, tgt_data, contrast_list, stage_name, args, subdir_name=""):
        if self.src_data:
        ...
        for contrast in contrast_list:
            ...
            self.writer = SummaryWriter(
                os.path.join(args.log_dir, subdir_name, stage_name,
                             run_name))
            encoder, classifier, val_score = self._adapt_train_test(cluster_src_loader, cluster_tgt_loader, contrast, args)
            performance_dict[contrast] = {'encoder': encoder, 'classifier': classifier, 'val_score': val_score}
