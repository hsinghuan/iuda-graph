...
class IWJAN():
    def __init__(self, encoder, classifier, src_data, device="cpu", oracle=False):
        self.device = device
        ...
        self.oracle = oracle
        self.class_num = 40
        self.source_label_distribution = (torch.sum(F.one_hot(self.src_data.y, num_classes=self.class_num), dim=0) / self.src_data.y.shape[0]).to(self.device)
    ...
    def _adapt_train_epoch(self, encoder, classifier, tgt_data, optimizer, lambda_coeff):
        ...
        ys_onehot = F.one_hot(self.src_data.y[self.src_data.train_mask], num_classes=self.class_num)
        ...
        cls_loss = torch.mean(F.nll_loss(F.log_softmax(src_y[self.src_data.train_mask], dim=1), self.src_data.y[self.src_data.train_mask], weight=self.class_weights, reduction='none') * weights) / self.class_num
        transfer_loss = self._minibatch_jmmd(src_f[self.src_data.train_mask], src_y[self.src_data.train_mask],
                                       tgt_f[tgt_data.train_mask], tgt_y[tgt_data.train_mask], weights)
    ...
    def _adapt_test_epoch(self, encoder, classifier, tgt_data, lambda_coeff):
        ...
        ys_onehot = F.one_hot(self.src_data.y[self.src_data.val_mask], num_classes=self.class_num)
        ys_onehot = ys_onehot.type(torch.FloatTensor).to(self.device)
        if self.oracle:
            weights = torch.mm(ys_onehot, self.true_weights)
        ...
        src_y, src_f = classifier(encoder(self.src_data.x, self.src_data.edge_index))
        tgt_y, tgt_f = classifier(encoder(tgt_data.x, tgt_data.edge_index))
        cls_loss = torch.mean(F.nll_loss(F.log_softmax(src_y[self.src_data.val_mask], dim=1), self.src_data.y[self.src_data.val_mask], weight=self.class_weights, reduction='none') * weights) / self.class_num
        transfer_loss = self._minibatch_jmmd(src_f[self.src_data.val_mask], src_y[self.src_data.val_mask],
                                       tgt_f[tgt_data.val_mask], tgt_y[tgt_data.val_mask], weights)
        loss = cls_loss + transfer_loss * lambda_coeff
    ...
    def _minibatch_jmmd(self, src_f, src_y, tgt_f, tgt_y, weights, batch_size=32):
        src_loader = torch.utils.data.DataLoader(tuple(zip(list(src_f), list(src_y), list(weights))), batch_size=batch_size,
                                                 shuffle=True)
        ...
        for i in range(len_dataloader):
            src_f, src_y, weights = src_iter.next()
            tgt_f, tgt_y = tgt_iter.next()
            if src_f.shape[0] != tgt_f.shape[0]:
                break

            total_transfer_loss += self.jmmd_loss((src_f, F.softmax(src_y, dim=1)), (tgt_f, F.softmax(tgt_y, dim=1)), weights=weights)
...
class JointMultipleKernelMaximumMeanDiscrepancy(nn.Module):
    ...
    def forward(self, z_s: torch.Tensor, z_t: torch.Tensor, weights:Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        weight_matrix = torch.ones_like(self.index_matrix)
        if weights is not None:
            weight_matrix[:batch_size, :batch_size] = torch.outer(weights.squeeze(1), weights.squeeze(1))
            weight_matrix[:batch_size, batch_size:] = weights.repeat(1, batch_size)
            weight_matrix[batch_size:, :batch_size] = weights.repeat(1, batch_size).transpose(0,1)


        # Add 2 / (n-1) to make up for the value on the diagonal
        # to ensure loss is positive in the non-linear version
        loss = (kernel_matrix * self.index_matrix * weight_matrix).sum() + 2. / float(batch_size - 1)
        return loss
